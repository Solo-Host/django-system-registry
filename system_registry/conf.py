...
DEFAULT_CACHE_PREFIX = "system_registry:"
...
def get_registry_definitions() -> RegistryDefinitions:
    ...
    provider = getattr(settings, "SYSTEM_REGISTRY_DEFINITIONS_PROVIDER", None)
    if provider is not None:
        loader = _coerce_provider(provider)
        definitions.update(loader())

    definitions.update(getattr(settings, "SYSTEM_REGISTRY_DEFINITIONS", {}))
    return definitions


def get_cache_prefix() -> str:
    return str(getattr(settings, "SYSTEM_REGISTRY_CACHE_PREFIX", DEFAULT_CACHE_PREFIX))


def get_cache_timeout() -> int:
    return int(getattr(settings, "SYSTEM_REGISTRY_CACHE_TIMEOUT", DEFAULT_CACHE_TIMEOUT))
...
def _coerce_provider(provider: str | DefinitionsProvider) -> DefinitionsProvider:
    ...
    if not callable(loaded):
        msg = "SYSTEM_REGISTRY_DEFINITIONS_PROVIDER must be callable."
        raise TypeError(msg)
